...
def bloco(ax, x, y, w, h, txt, fc=None, ec=None, fs=8):
    fc = fc or COR["bloco_f"]
    ec = ec or COR["bloco_e"]
    ax.add_patch(mpatches.FancyBboxPatch(
        (x - w / 2, y - h / 2), w, h,
        boxstyle="round,pad=0.04", facecolor=fc, edgecolor=ec, lw=1.5, zorder=3))
...
def linha(ax, x1, y1, x2, y2):
    ax.plot([x1, x2], [y1, y2], color=COR["bloco_e"], lw=1.4, zorder=2)

def ponto(ax, x, y):
    ax.plot(x, y, "o", color=COR["bloco_e"], ms=4, zorder=5)

def circulo(ax, x, y, r=0.25):
    ax.add_patch(plt.Circle((x, y), r, fc="white", ec=COR["bloco_e"], lw=1.4, zorder=3))
...
# Diagrama MIMO
fig_mimo, ax_mimo = plt.subplots(figsize=(5, 2.2))
ax_mimo.set_xlim(-0.5, 10.5)
ax_mimo.set_ylim(-0.3, 4.3)
ax_mimo.set_aspect("equal")
ax_mimo.axis("off")
...
ax_mimo.text(4.0, 2.0, "Sistema", ha="center", va="center", fontsize=11, zorder=4)

for lbl, y in [(r"$x_1(t)$", 3.5), (r"$x_2(t)$", 2.5), (r"$x_j(t)$", 0.5)]:
    ax_mimo.text(0.3, y, lbl, ha="center", va="center", fontsize=9.5)
    ax_mimo.annotate("", xy=(2.9, y), xytext=(1.0, y),
        arrowprops=dict(arrowstyle="->", color=COR["bloco_e"], lw=1.4, shrinkA=0, shrinkB=0), zorder=2)

ax_mimo.text(0.3, 1.5, r"$\vdots$", ha="center", va="center", fontsize=13, color="black")

for lbl, y in [(r"$y_1(t)$", 3.5), (r"$y_2(t)$", 2.5), (r"$y_k(t)$", 0.5)]:
    ax_mimo.annotate("", xy=(7.1, y), xytext=(5.1, y),
        arrowprops=dict(arrowstyle="->", color=COR["bloco_e"], lw=1.4, shrinkA=0, shrinkB=0), zorder=2)
    ax_mimo.text(7.7, y, lbl, ha="center", va="center", fontsize=9.5)

ax_mimo.text(9.7, 1.5, r"$\vdots$", ha="center", va="center", fontsize=13, color="black")
...
fig_tam, axs_tam = plt.subplots(1, 2, figsize=(8, 3))

t1 = np.linspace(0, 10, 300)
y_lim = np.exp(-(t1 - 3) ** 2 / 0.5)
axs_tam[0].plot(t1, y_lim, color=COR["sinal"], lw=1.8)
axs_tam[0].set_title(r"Sinal de energia  ($E_x < \infty$, $P_x = 0$)", fontsize=9)
axs_tam[0].fill_between(t1, y_lim, alpha=0.15, color=COR["sinal"])
axs_tam[0].set_yticks([])
estilo(axs_tam[0])

t2 = np.linspace(0, 10, 1000)
y_pot = 0.3 * np.sin(2 * np.pi * 0.4 * t2) + 0.2 * np.cos(2 * np.pi * 1.1 * t2) + 0.15 * np.sin(2 * np.pi * 2.3 * t2)
axs_tam[1].plot(t2, y_pot, color=COR["sinal"], lw=1.8)
axs_tam[1].set_title(r"Sinal de potência  ($P_x < \infty$, $E_x = \infty$)", fontsize=9)
axs_tam[1].set_yticks([])
estilo(axs_tam[1])

plt.suptitle("Classificação pelo Tamanho do Sinal", fontsize=10, fontweight="bold")
plt.tight_layout()
st.pyplot(fig_tam)
plt.close(fig_tam)
...
O menor $T_0$ que satisfaz essa condição é o **período fundamental**. Um sinal periódico é
definido para todo $t \in (-\infty, +\infty)$.
""")

# Natureza dos sinais
fig_nat, axs_nat = plt.subplots(2, 2, figsize=(8, 4.5))
fig_nat.suptitle("Natureza dos Sinais", fontsize=11, fontweight="bold")

xc = np.linspace(0, 4, 400)
xd = np.arange(0, 4.25, 0.25)
sq_c = np.where((np.floor(xc) % 2) == 0,  1.0, -1.0)
sq_d = np.where((np.floor(xd) % 2) == 0,  1.0, -1.0)

axs_nat[0, 0].set_title("Analógico", fontsize=10, fontweight="bold")
axs_nat[0, 1].set_title("Digital",   fontsize=10, fontweight="bold")
axs_nat[0, 0].set_ylabel("Contínuo", fontsize=10, fontweight="bold", labelpad=8)
axs_nat[1, 0].set_ylabel("Discreto", fontsize=10, fontweight="bold", labelpad=8)

axs_nat[0, 0].plot(xc, xc ** 2 / 8, "k", lw=1.8)
axs_nat[0, 0].set_xlabel("$t$", fontsize=8); axs_nat[0, 0].set_yticks([])
axs_nat[0, 0].spines[["right", "top"]].set_visible(False)

axs_nat[0, 1].plot(xc, sq_c, "k", lw=1.8)
axs_nat[0, 1].set_xlabel("$t$", fontsize=8); axs_nat[0, 1].set_yticks([])
axs_nat[0, 1].spines[["right", "top"]].set_visible(False)

axs_nat[1, 0].stem(xd, xd / 4, linefmt="k-", markerfmt="ko", basefmt="k-")
axs_nat[1, 0].set_xlabel("$n$", fontsize=8)
axs_nat[1, 0].spines[["right", "top"]].set_visible(False)

axs_nat[1, 1].stem(xd, sq_d, linefmt="k-", markerfmt="ko", basefmt="k-")
axs_nat[1, 1].set_xlabel("$n$", fontsize=8)
...
# Periodicidade
fig_per, axs_per = plt.subplots(1, 2, figsize=(8, 3.2))
fig_per.suptitle("Periodicidade de Sinais", fontsize=11, fontweight="bold")

t_per = np.linspace(-5, 5, 1000)
y_per = np.sin(np.pi * t_per) + 0.35 * np.sin(3 * np.pi * t_per)
axs_per[0].plot(t_per, y_per, color=COR["sinal"], lw=1.8)
axs_per[0].set_title("Periódico", fontsize=10)
...
axs_per[0].text(0, -1.75, "$T_0$", ha="center", fontsize=10)
axs_per[0].set_yticks([]); axs_per[0].set_xlabel("$t$", fontsize=9)
...
axs_per[1].plot(t_aper, y_aper, color="coral", lw=1.8)
axs_per[1].set_title("Aperiódico", fontsize=10)
axs_per[1].set_yticks([]); axs_per[1].set_xlabel("$t$", fontsize=9)
...
fig_caus, axs_caus = plt.subplots(1, 3, figsize=(9, 3))
fig_caus.suptitle("Causalidade de Sinais", fontsize=11, fontweight="bold")

y_c = np.where(t_caus >= 0, np.exp(-t_caus) * np.sin(4 * t_caus), 0)
axs_caus[0].plot(t_caus, y_c, color=COR["sinal"], lw=1.8)
axs_caus[0].set_title("Causal\n$x(t)=0,\\; t<0$", fontsize=9)
axs_caus[0].axvline(0, color="k", lw=0.8, ls="--")
axs_caus[0].fill_betweenx([-1.1, 1.1], -4, 0, alpha=0.06, color="gray")
axs_caus[0].set_yticks([]); axs_caus[0].set_xlabel("$t$", fontsize=9)
axs_caus[0].spines[["right", "top"]].set_visible(False); axs_caus[0].set_ylim(-1.1, 1.1)

y_nc = np.exp(-0.3 * np.abs(t_caus)) * np.sin(3 * t_caus)
axs_caus[1].plot(t_caus, y_nc, color="seagreen", lw=1.8)
axs_caus[1].set_title("Não-causal\n$x(t)\\neq 0$ para $t<0$", fontsize=9)
axs_caus[1].axvline(0, color="k", lw=0.8, ls="--")
axs_caus[1].set_yticks([]); axs_caus[1].set_xlabel("$t$", fontsize=9)
...
axs_caus[2].plot(t_caus, y_ac, color="darkorange", lw=1.8)
axs_caus[2].set_title("Anti-causal\n$x(t)=0,\\; t\\geq 0$", fontsize=9)
axs_caus[2].axvline(0, color="k", lw=0.8, ls="--")
axs_caus[2].fill_betweenx([-1.1, 1.1], 0, 4, alpha=0.06, color="gray")
axs_caus[2].set_yticks([]); axs_caus[2].set_xlabel("$t$", fontsize=9)
...
fig_desl, axs_desl = plt.subplots(3, 1, figsize=(6, 5), sharex=True)
fig_desl.suptitle("Deslocamento Temporal", fontsize=10, fontweight="bold")
...
for ax, (sig, tit, cor) in zip(axs_desl, [
    ...
    ax.plot(x_d, sig, color=cor, lw=1.8)
    ax.axvline(0, color="k", lw=0.7)
    ax.spines[["right", "top", "left"]].set_visible(False)
    ax.set_yticks([])
    ax.text(0.02, 0.88, tit, transform=ax.transAxes, fontsize=8.5, va="top")

axs_desl[-1].set_xlabel("$t$", fontsize=9)
plt.tight_layout()
st.pyplot(fig_desl)
plt.close(fig_desl)
...
$$\phi(t) = x(at), \quad a > 1 \quad \Rightarrow \text{compressão (mais rápido)}$$

$$\phi(t) = x\!\left(\frac{t}{a}\right), \quad a > 1 \quad \Rightarrow \text{expansão (mais lento)}$$
""")
...
fig_esc, axs_esc = plt.subplots(3, 1, figsize=(6, 5), sharex=True)
fig_esc.suptitle("Escalamento Temporal", fontsize=10, fontweight="bold")
...
for ax, (sig, tit) in zip(axs_esc, [
    ...
    ax.plot(x_e, np.clip(sig, -0.3, 1.3), "k", lw=1.8)
    ax.axvline(0, color="k", lw=0.7); ax.axhline(0, color="k", lw=0.7)
    ax.spines[["right", "top", "left"]].set_visible(False)
    ax.set_yticks([]); ax.set_ylim(-0.45, 1.45)
    ax.text(0.02, 0.95, tit, transform=ax.transAxes, fontsize=8.5, va="top")

axs_esc[-1].set_xlabel("$t$", fontsize=9)
plt.tight_layout()
st.pyplot(fig_esc)
plt.close(fig_esc)
...
Substitui $t$ por $-t$: espelha o sinal em torno do **eixo de amplitude** ($t=0$).
Não confundir com a inversão de amplitude $\phi(t) = -x(t)$, que espelha em torno do **eixo do tempo**.
""")
...
fig_rev, axs_rev = plt.subplots(1, 2, figsize=(7, 3))
for ax, (xarg, tit) in zip(axs_rev, [
    (x_r,  "$x(t)$"),
    (-x_r, "$x(-t)$ — reversão"),
]):
    ax.plot(x_r, f_rev(xarg), "k", lw=1.8)
    ax.set_title(tit, fontsize=9)
    ax.axvline(0, color="k", lw=0.7); ax.axhline(0, color="k", lw=0.7)
    ax.spines[["right", "top"]].set_visible(False)
    ax.set_yticks([]); ax.set_xticks([]); ax.set_ylim(-4, 6)

plt.suptitle("Reversão Temporal", fontsize=10, fontweight="bold")
plt.tight_layout()
st.pyplot(fig_rev)
plt.close(fig_rev)
...
fig_pi, axs_pi = plt.subplots(1, 3, figsize=(9, 3.2), sharex=True, sharey=True)
for ax, (y, tit, cor) in zip(axs_pi, [
    (x_pi,  "$x(t)$ — original",  COR["sinal"]),
    (xe_pi, "$x_e(t)$ — par",    "seagreen"),
    (xo_pi, "$x_o(t)$ — ímpar",  COR["saida"]),
]):
    ax.plot(t_pi, y, color=cor, lw=1.8)
    ax.set_title(tit, fontsize=9)
    ax.axhline(0, color="k", lw=0.6); ax.axvline(0, color="k", lw=0.6)
    ax.spines[["right", "top"]].set_visible(False)
    ax.set_yticks([]); ax.set_xlabel("$t$", fontsize=8)

plt.suptitle("Componentes Par e Ímpar", fontsize=10, fontweight="bold")
plt.tight_layout()
st.pyplot(fig_pi)
plt.close(fig_pi)
...
fig_sg, axs_sg = plt.subplots(4, 1, figsize=(6, 7), sharex=True)
fig_sg.suptitle("Funções Singulares (atraso $t_0=1$)", fontsize=10, fontweight="bold")
...
for ax, (y, cor, lb) in zip(axs_sg, [
    ...
    ax.plot(t_sg, y, color=cor, lw=1.8, label=lb)
    ax.set_ylabel("Amp.", fontsize=8)
    ax.legend(loc="upper left")
    ax.spines[["right", "top"]].set_visible(False)

axs_sg[-1].set_xlabel("Tempo (s)", fontsize=9)
plt.tight_layout()
st.pyplot(fig_sg)
plt.close(fig_sg)
...
fig_ex, axs_ex = plt.subplots(2, 3, figsize=(11, 5), sharex=True)
fig_ex.suptitle(r"Funções Exponenciais — casos de $e^{st}$", fontsize=11, fontweight="bold")
...
for nome, cor, titulo, ax in itens:
    ax.plot(t_ex, np.clip(exps[nome], -3, 3), color=cor, lw=1.8)
...
for ax in [axs_ex[0, 0], axs_ex[1, 0]]:
...
env   = np.exp(-0.5 * t_env)
sam   = np.exp(-0.5 * t_env) * np.cos(5 * t_env)
...
ax_env.plot(t_env, sam,  color=COR["degrau"], lw=1.8, label="Senoide amortecida")
ax_env.axhline(0, color="k", lw=0.5)
ax_env.set_title("Senoide amortecida e envelope exponencial", fontsize=10)
ax_env.legend()
...
# ═══════════════════════════════════════════════════════════════════════════════
# SEÇÃO 9 — PRINCÍPIO DA SUPERPOSIÇÃO
# ═══════════════════════════════════════════════════════════════════════════════
...
x1_sp = np.sin(2 * np.pi * t_sp)
x2_sp = 0.5 * np.cos(4 * np.pi * t_sp)

fig_sp, axs_sp = plt.subplots(3, 1, figsize=(7, 6), sharex=True)
fig_sp.suptitle(r"Princípio da Superposição — $y = 2x$", fontsize=10, fontweight="bold")

axs_sp[0].plot(t_sp, x1_sp,         COR["degrau"], lw=1.8, label="$x_1(t)$")
axs_sp[0].plot(t_sp, x2_sp,         COR["saida"],  lw=1.8, label="$x_2(t)$")
axs_sp[0].plot(t_sp, x1_sp + x2_sp, "k--",         lw=1.2, label="$x_1+x_2$")
axs_sp[0].set_ylabel("Entradas", fontsize=8); axs_sp[0].legend()

axs_sp[1].plot(t_sp, 2 * x1_sp, COR["degrau"], lw=1.8, label="$y_1 = 2x_1$")
axs_sp[1].plot(t_sp, 2 * x2_sp, COR["saida"],  lw=1.8, label="$y_2 = 2x_2$")
axs_sp[1].set_ylabel("Saídas indiv.", fontsize=8); axs_sp[1].legend()

axs_sp[2].plot(t_sp, 2 * (x1_sp + x2_sp), "k-",           lw=2.5, label="$2(x_1+x_2)$  [entrada somada]")
axs_sp[2].plot(t_sp, 2 * x1_sp + 2 * x2_sp, "darkorange",  lw=1.4, ls="--", label="$y_1+y_2$  [saídas somadas]")
axs_sp[2].set_ylabel("Saída total", fontsize=8)
axs_sp[2].set_xlabel("Tempo (s)", fontsize=9); axs_sp[2].legend()
...
fig_est, axs_est = plt.subplots(3, 1, figsize=(7, 6), sharex=True)
fig_est.suptitle(r"Estabilidade de Sistemas LIT — Resposta Natural $y_n(t)$",
...
for ax, (y, cor, tit) in zip(axs_est, [
    ...
    (np.cos(10 * t_est),                          COR["marginal"], r"Marginalmente estável — $y_n(t)$ constante"),
]):
    ax.plot(t_est, np.clip(y, -2.5, 2.5), color=cor, lw=1.8)
    ax.axhline(0, color="k", lw=0.6, ls="--")
    ax.set_ylabel("$y_n(t)$", fontsize=8); ax.set_title(tit, fontsize=9)
    ax.spines[["right", "top"]].set_visible(False)
    ax.set_ylim(-2.5, 2.5)

axs_est[-1].set_xlabel("Tempo (s)", fontsize=9); axs_est[-1].set_xlim(0, 5)
plt.tight_layout()
st.pyplot(fig_est)
plt.close(fig_est)
...
fig_ma, ax_ma = plt.subplots(figsize=(10, 2.0))
ax_ma.set_xlim(-0.5, 11.5); ax_ma.set_ylim(-0.7, 0.7); ax_ma.axis("off")
ax_ma.set_title("Sistema em Malha Aberta", fontsize=10, fontweight="bold", pad=5)

cx = [0.9, 3.3, 5.7, 8.1]; w_b = 1.8; h_b = 0.46

x_rd = cx[1] - w_b / 2 - 1.3
ax_ma.text(x_rd, 0, "Resposta\ndesejada", ha="center", va="center", fontsize=8)
...
**Vantagens em relação à malha aberta:** rejeição de perturbações, robustez a incertezas do modelo
e redução do erro estacionário.
""")

# Diagrama malha fechada
fig_mf, ax_mf = plt.subplots(figsize=(13, 4.5))
ax_mf.set_aspect("equal")
ax_mf.set_xlim(-0.8, 14.5); ax_mf.set_ylim(-2.8, 1.6); ax_mf.axis("off")
ax_mf.set_title("Sistema em Malha Fechada", fontsize=10, fontweight="bold", pad=5)

def _arr_mf(ax, x1, y1, x2, y2, lb="", ldy=0.13):
    ax.annotate("", xy=(x2, y2), xytext=(x1, y1),
        arrowprops=dict(arrowstyle="->", color=COR["bloco_e"], lw=1.4, shrinkA=0, shrinkB=0), zorder=2)
    if lb:
        ax.text((x1+x2)/2, (y1+y2)/2+ldy, lb, ha="center", fontsize=7.5, zorder=5)

def _lin_mf(ax, x1, y1, x2, y2):
    ax.plot([x1, x2], [y1, y2], color=COR["bloco_e"], lw=1.4, zorder=2)

def _dot_mf(ax, x, y):
    ax.plot(x, y, "o", color=COR["bloco_e"], ms=5, zorder=5)

def _som_mf(ax, x, y, r=0.28):
    ax.add_patch(mpatches.Circle((x, y), r, fc="white", ec=COR["bloco_e"], lw=1.5, zorder=3))

def _pm_mf(ax, x, y, s, c):
    ax.text(x, y, s, ha="center", va="center", fontsize=9, color=c, fontweight="bold", zorder=6)

w_b2, h_b2, r_s = 1.8, 0.46, 0.28
x_rd2=-0.3; x_s1=1.0; x_ct=3.1; x_at=5.3; x_sp=7.2; x_pr=9.3; x_dt=11.2
y_fb=-1.8; x_sr=9.3; x_sn=5.3

ax_mf.text(x_rd2, 0, "Resposta\ndesejada", ha="center", va="center", fontsize=8)
_arr_mf(ax_mf, x_rd2+0.55, 0, x_s1-r_s, 0)
_som_mf(ax_mf, x_s1, 0)
_pm_mf(ax_mf, x_s1-r_s-0.16, +0.22, "+", "seagreen")
_pm_mf(ax_mf, x_s1+0.16, -r_s-0.18, "−", "crimson")

_arr_mf(ax_mf, x_s1+r_s, 0, x_ct-w_b2/2, 0, "erro", ldy=0.14)
bloco(ax_mf, x_ct, 0, w_b2, h_b2, "Controlador")
_arr_mf(ax_mf, x_ct+w_b2/2, 0, x_at-w_b2/2, 0)
bloco(ax_mf, x_at, 0, w_b2, h_b2, "Atuador")
_arr_mf(ax_mf, x_at+w_b2/2, 0, x_sp-r_s, 0)

_som_mf(ax_mf, x_sp, 0)
_pm_mf(ax_mf, x_sp-r_s-0.16, +0.22, "+", "seagreen")
_pm_mf(ax_mf, x_sp+0.16, r_s+0.16, "+", "seagreen")
ax_mf.annotate("perturbação", xy=(x_sp, r_s), xytext=(x_sp, 1.15),
            arrowprops=dict(arrowstyle="->", color="gray", lw=1, shrinkA=0, shrinkB=0),
            fontsize=7, color="gray", ha="center", va="bottom")
_arr_mf(ax_mf, x_sp+r_s, 0, x_pr-w_b2/2, 0)
bloco(ax_mf, x_pr, 0, w_b2, h_b2, "Processo")
_lin_mf(ax_mf, x_pr+w_b2/2, 0, x_dt, 0)
_dot_mf(ax_mf, x_dt, 0)
_arr_mf(ax_mf, x_dt, 0, 13.0, 0)
ax_mf.text(13.15, 0, "Saída\nreal", ha="left", va="center", fontsize=8)

_lin_mf(ax_mf, x_dt, 0, x_dt, y_fb)
_arr_mf(ax_mf, x_dt, y_fb, x_sr+r_s, y_fb)

_som_mf(ax_mf, x_sr, y_fb)
_pm_mf(ax_mf, x_sr+r_s+0.16, y_fb+0.22, "+", "seagreen")
_pm_mf(ax_mf, x_sr+0.16, y_fb-r_s-0.18, "+", "seagreen")
ax_mf.annotate("ruído de\nmedição", xy=(x_sr, y_fb-r_s),
            xytext=(x_sr, y_fb-1.15),
            arrowprops=dict(arrowstyle="->", color="gray", lw=1, shrinkA=0, shrinkB=0),
            fontsize=7, color="gray", ha="center", va="top")

_arr_mf(ax_mf, x_sr-r_s, y_fb, x_sn+w_b2/2, y_fb)
bloco(ax_mf, x_sn, y_fb, w_b2, h_b2, "Sensor")

_lin_mf(ax_mf, x_sn-w_b2/2, y_fb, x_s1, y_fb)
_arr_mf(ax_mf, x_s1, y_fb, x_s1, -r_s)

ax_mf.text((x_dt+x_sr)/2+0.3, y_fb-0.28, "Saída\nmensurada",
...
**Objetivos funcionais:** amplificação de potência, controle remoto, adaptação da forma da entrada
e compensação de perturbações.
...
fig_tr, ax_tr = plt.subplots(figsize=(8, 3.8))
ax_tr.plot(t_tr, y_tr, color=COR["saida"], lw=2, label="Saída $y(t)$")
ax_tr.axhline(1, color="gray", ls="--", lw=1, label="Sinal de entrada (referência)")
...
y_final = float(y_tr[-1])
x_err   = t_tr[-1] * 0.88
ax_tr.annotate("", xy=(x_err, y_final), xytext=(x_err, 1.0),
            arrowprops=dict(arrowstyle="<->", color="purple", lw=1.2))
ax_tr.text(x_err + 0.06, (y_final + 1.0) / 2, "Erro\nestacionário",
        fontsize=7.5, color="purple", ha="left", va="center")

ax_tr.text(t_lim / 2, 0.20, "Resposta\ntransitória",
        ha="center", fontsize=8, color="royalblue", fontweight="bold")
ax_tr.text(t_lim + (t_tr[-1] - t_lim) * 0.38, 0.20, "Resposta\nestacionária",
        ha="center", fontsize=8, color="seagreen", fontweight="bold")

ax_tr.set_xlabel("Tempo (s)", fontsize=9); ax_tr.set_ylabel("Amplitude", fontsize=9)
ax_tr.set_title("Resposta transitória e estacionária ao degrau", fontsize=10, fontweight="bold")
...
fig_ent, axs_ent = plt.subplots(1, 5, figsize=(13, 3.2))
fig_ent.suptitle("Sinais de Entrada para Análise de Desempenho", fontsize=10, fontweight="bold")
...
for ax, y, nome, cor in zip(axs_ent,
    ...
    ax.plot(t_ent, np.clip(y, -1.5, 3.5), color=cor, lw=1.8)
    ax.set_title(nome, fontsize=8)
    ax.set_xlabel("$t$", fontsize=8); ax.axhline(0, color="k", lw=0.5)
    ax.spines[["right", "top"]].set_visible(False)

axs_ent[0].set_ylabel("Amplitude", fontsize=8)
plt.tight_layout()
st.pyplot(fig_ent)
plt.close(fig_ent)
...
O sinal **$-$** no somador (subtração) corresponde a **realimentação negativa** → denominador $1 + G(s)F(s)$.
O sinal **$+$** no somador (adição) corresponde a **realimentação positiva** → denominador $1 - G(s)F(s)$.

### 12.4 Álgebra de Blocos

Técnica de simplificação de diagramas complexos por movimentação e combinação de blocos e
pontos de soma/ramificação.
""")

# Funções auxiliares álgebra de blocos
...
def _a(ax, x1, y1, x2, y2):
    ax.annotate("", xy=(x2, y2), xytext=(x1, y1),
        arrowprops=dict(arrowstyle="->", color=COR["bloco_e"], lw=1.3, shrinkA=0, shrinkB=0), zorder=2)

def _l(ax, x1, y1, x2, y2):
    ax.plot([x1, x2], [y1, y2], color=COR["bloco_e"], lw=1.3, zorder=2)

def _s(ax, x, y, r=0.22):
    ax.add_patch(mpatches.Circle((x, y), r, fc="white", ec=COR["bloco_e"], lw=1.3, zorder=3))
...
def _setup(ax, titulo):
    ...
    ax.text(5.5, 0.0, "⟺", ha="center", va="center", fontsize=15, color="dimgray")
    ax.text(0.1, 1.52, "Original:",    ha="left", va="top", fontsize=7.5, color="dimgray")
    ax.text(5.7, 1.52, "Equivalente:", ha="left", va="top", fontsize=7.5, color="dimgray")

wb = 1.3; hb = 0.40; r = 0.22

# Cascata
fig_cas, ax_cas = plt.subplots(figsize=(7, 1.8))
ax_cas.set_xlim(0, 10); ax_cas.set_ylim(-0.6, 0.7); ax_cas.axis("off")
ax_cas.set_title("Cascata (Série)", fontsize=10, fontweight="bold", pad=4)
ax_cas.text(0.5, 0.98, r"$G_{eq}(s)=G_1(s)\cdot G_2(s)\cdots G_n(s)$",
        transform=ax_cas.transAxes, ha="center", va="top", fontsize=9)

W, H = 1.5, 0.42
cx_c = [1.8, 4.2, 7.8]

ax_cas.text(0.3, 0, "$X(s)$", ha="center", va="center", fontsize=9)
_a(ax_cas, 0.6, 0, cx_c[0]-W/2, 0)
for x, lb in zip(cx_c, ["$G_1(s)$", "$G_2(s)$", "$G_n(s)$"]):
    _blk(ax_cas, x, 0, W, H, lb)
_a(ax_cas, cx_c[0]+W/2, 0, cx_c[1]-W/2, 0)
_a(ax_cas, cx_c[1]+W/2, 0, cx_c[1]+W/2+0.5, 0)
ax_cas.text(6.3, 0, r"$\cdots$", ha="center", va="center", fontsize=14)
_a(ax_cas, 6.65, 0, cx_c[2]-W/2, 0)
_a(ax_cas, cx_c[2]+W/2, 0, 9.4, 0)
ax_cas.text(9.7, 0, "$Y(s)$", ha="center", va="center", fontsize=9)
...
# Paralelo
fig_par2, ax_par2 = plt.subplots(figsize=(7, 3.5))
...
W2, H2 = 2.2, 0.44
xN = 1.4; xBc = 5.0; xS = 8.6
ys = [1.1, 0.0, -1.1]

ax_par2.text(0.3, 0, "$X(s)$", ha="center", va="center", fontsize=9)
_l(ax_par2, 0.6, 0, xN, 0)
ponto(ax_par2, xN, 0)

for i, y in enumerate(ys):
    _l(ax_par2, xN, 0, xN, y)
    _a(ax_par2, xN, y, xBc-W2/2, y)
    _blk(ax_par2, xBc, y, W2, H2, f"$G_{i+1}(s)$")
    _l(ax_par2, xBc+W2/2, y, xS, y)

_l(ax_par2, xS, ys[0], xS, ys[-1])
ponto(ax_par2, xS, 0)
_a(ax_par2, xS, 0, 9.4, 0)
ax_par2.text(9.7, 0, "$Y(s)$", ha="center", va="center", fontsize=9)
...
# Realimentação
fig_real, ax_real = plt.subplots(figsize=(7, 3.2))
ax_real.set_aspect("equal")
ax_real.set_xlim(0, 10)
ax_real.set_ylim(-1.7, 0.9)
ax_real.axis("off")
...
ax_real.text(xR2, 0, "$R(s)$", ha="center", va="center", fontsize=9)
...
ax_real.text((xS2+rs2+xG2-Wf/2)/2, 0.14, "$E(s)$", ha="center", fontsize=8)
_blk(ax_real, xG2, 0, Wf, Hf, "$G(s)$")
_l(ax_real, xG2+Wf/2, 0, xD2, 0)
ponto(ax_real, xD2, 0)
_a(ax_real, xD2, 0, xY2, 0)
ax_real.text(xY2+0.3, 0, "$Y(s)$", ha="center", va="center", fontsize=9)

_l(ax_real, xD2, 0, xD2, yF2)
_blk(ax_real, xG2, yF2, Wf, Hf, "$F(s)$")
_a(ax_real, xD2, yF2, xG2+Wf/2, yF2)
_l(ax_real, xG2-Wf/2, yF2, xS2, yF2)
...
_s(ax_alg1, 8.67, 0.7)
_pm2(ax_alg1, 8.67-r-0.22, 0.7+0.32, "±", "black")
_a(ax_alg1, 8.89, 0.7, 9.4, 0.7); _t(ax_alg1, 9.65, 0.7, "$Y(s)$")
_t(ax_alg1, 5.7, -0.42, "$X(s)$", fs=7.5)
_blk(ax_alg1, 7.3, -0.42, wb, hb, "$G(s)$", fs=7)
_a(ax_alg1, 6.15, -0.42, 6.65, -0.42)
_a(ax_alg1, 7.95, -0.42, 8.67, -0.42)
...
_t(ax_alg2, 1.32, -1.05, "$X(s)$", fs=7.5)
_a(ax_alg2, 1.32, -0.83, 1.32, -0.65)
_a(ax_alg2, 1.32, -0.25, 1.32, 0.48)

_t(ax_alg2, 5.7, 0.7, "$R(s)$"); _a(ax_alg2, 6.15, 0.7, 6.65, 0.7)
_blk(ax_alg2, 7.3, 0.7, wb, hb, "$G(s)$"); _a(ax_alg2, 7.95, 0.7, 8.45, 0.7)
_s(ax_alg2, 8.67, 0.7)
_pm2(ax_alg2, 8.67-r-0.22, 0.7+0.32, "±", "black")
_a(ax_alg2, 8.89, 0.7, 9.4, 0.7); _t(ax_alg2, 9.65, 0.7, "$Y(s)$")
_t(ax_alg2, 8.67, -0.42, "$X(s)$", fs=7.5)
_a(ax_alg2, 8.67, -0.20, 8.67, 0.48)
...
# Álgebra 3
fig_alg3, ax_alg3 = plt.subplots(figsize=(9, 4.0))
...
# Álgebra 4
fig_alg4, ax_alg4 = plt.subplots(figsize=(9, 4.2))
...
fig_red, ax_red = plt.subplots(figsize=(7, 3.5))
...
ax_red.set_title(r"$G(s)=10/(s+2)$, $H(s)=1$ $\Rightarrow$ $T(s)=10/(s+12)$", fontsize=9)
ax_red.legend(); ax_red.set_xlim(0, t_red[-1])
plt.tight_layout()
st.pyplot(fig_red)
plt.close(fig_red)
